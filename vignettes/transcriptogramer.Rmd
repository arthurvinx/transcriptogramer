...
---
**Authors**: Diego Morais [aut, cre], Rodrigo Dalmolin [aut]<br />
...
protein network data, with combined scores equal or above 700, 800 and 900
(see Hs900, Hs800, Hs700, Mm900, Mm800, Mm700, Sc900, Sc800, Sc700, Rn900,
Rn800 and Rn700 datasets).
...
# Quick start
The first step is to create an Transcriptogram object by running the
**transcriptogram.preprocess()** function. This example uses a subset of the
Homo sapiens protein network data, from STRINGdb release 10.5, containing only
associations of proteins of combined score equal or above 900 (see **Hs900** and
...
centered at a protein, whose radius changes the output. The Transctiprogram
...
DE slot of the Transcriptogram object, its content can be seen using the getSlot
method. By default, the p-values are adjusted by the Benjamini-Hochberg
procedure.
...
display graphs of the differentially expressed clusters. This method returns an
object of the RedPort Class, allowing interactions via console. The RedeR
package requires the Java Runtime Environment (>= 6). This method may take some
time depending on the number of clusters, and nodes per cluster.
